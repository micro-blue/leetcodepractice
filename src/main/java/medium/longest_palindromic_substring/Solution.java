    ...
    public String longestPalindrome(String s) {
        ...
        for (int i = 0; i < chars.length; i++) {
            if(curFirstIndex==chars.length||curSecondIndex==chars.length)
                break;
            //该分支处理偶数情况
            if(curFirstIndex>=0&&curSecondIndex<chars.length&&chars[curFirstIndex]==chars[curSecondIndex]){
                curLength=0;
                ...
                while (tempHead>=0&&tempTail<chars.length&&chars[tempHead]==chars[tempTail]){
                    curLength+=2;
                    tempHead--;
                    tempTail++;
                }
            ...
            //该分支处理奇数情况
            if(curFirstIndex>=0&&curSecondIndex+1<chars.length&&chars[curFirstIndex]==chars[curSecondIndex+1]){
                curLength=1;
                int tempHead=curFirstIndex;
